...
- Provider → webhook endpoint → tenant ledger: unauthenticated network payloads cross a custom HMAC
  check, JSON parser, event replay lookup and event-type dispatch (`server.js`, `webhook`,
  `applyWebhook`).
...
| Authenticated accounting APIs | Session cookie plus CSRF   | User → Folio → tenant ledger        | Broad mutation set with central role mapping                    | `server.js:apiRequest/api/requiredPermission`                         |
| Webhooks                      | Public HTTPS POST          | Provider → Folio                    | Custom HMAC, tenant slug, event ID and payload                  | `server.js:webhook/applyWebhook`                                      |
...
| TM-003    | Malicious tenant member or coding defect          | One route/repository omits tenant boundary                | Supplies another tenant's object ID or causes wrong cached ledger selection                            | Financial/PII exfiltration or mutation across customers        | Tenant ledgers, attachments, audit data           | Session-selected org, separate files, pre-migration tenant binding, non-null `org_id`, prepared queries (`server.js:tenantLedger`; `lib/db.js:assertTenantBinding`; `test/tenancy.test.js`)                                                                                                                                                    | Large route surface; not every object route has generated cross-tenant coverage; shared platform DB remains critical                                 | Add an authorization manifest and generated negative test for every route/object; key cache by verified org and path; add database-path ownership assertion on every open       | Cross-org identifier probes, tenant mismatch failures, unusual 404 patterns                       | Low after controls              | High            | high     |
| TM-004    | Remote attacker with leaked/shared webhook secret | Provider secret is known or provider channel compromised  | Replays or forges HMAC payload for a chosen tenant slug                                                | Unauthorized cash, invoices, payroll or expense journals       | Ledgers, webhook audit, cash/AR                   | Stripe authenticates the exact raw body and signed timestamp with a bounded tolerance and rotation signatures; all sources have constant-time checks and body caps; tenant inbox atomically commits replay key, payload hash, result and financial effect (`lib/webhook-verification.js`; `lib/db.js:applyExternalEvent`; `server.js:webhook`) | Payroll/expense signatures remain Folio-specific; secrets are provider-global, not connection/tenant scoped; request processing is synchronous       | Implement remaining provider-native verification; use per-connection secrets and account binding; add a fast-ack queue/worker while preserving tenant-transactional application | Invalid signatures, stale timestamps, duplicate/event-account mismatch and webhook posting alerts | Medium until native integration | High            | high     |
