...
| TM-004    | Remote attacker with leaked/shared webhook secret | Provider secret is known or provider channel compromised  | Replays or forges HMAC payload for a chosen tenant slug                                                | Unauthorized cash, invoices, payroll or expense journals       | Ledgers, webhook audit, cash/AR                   | HMAC with constant-time comparison, body cap, supported types, event ID replay record (`server.js:webhook`)                                                                                                                          | Custom signature lacks provider timestamp/tolerance; secrets are provider-global, not connection/tenant scoped; no durable queue before processing   | Implement each provider's native signature scheme and timestamp window; per-connection secret; durable inbox transaction; bind event account to connection/tenant         | Invalid signatures, stale timestamps, duplicate/event-account mismatch and webhook posting alerts | Medium until native integration | High            | high     |
