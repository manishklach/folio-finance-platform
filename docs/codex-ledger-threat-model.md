...
ceremony, MFA/SSO, Plaid Link/provider webhook validation, live cloud controls, and independent
...
- Live provider sandbox exchanges, provider-native webhook verification and certification are
...
| Provider accounting mapping   | Admin/operator session     | Staged record → mapping → draft     | Versioned fields, fingerprinted approval, idempotency and no auto-post | `lib/integrations.js:previewIntegrationRecordApplication/applyIntegrationRecord` |
...
| TM-004    | Remote attacker with leaked/shared webhook secret | Provider secret is known or provider channel compromised  | Replays or forges HMAC payload for a chosen tenant slug                                                | Unauthorized cash, invoices, payroll or expense journals       | Ledgers, webhook audit, cash/AR                   | Stripe authenticates exact raw body/timestamp with connection-scoped secret and account binding; production rejects slug-only endpoints; verified events durably fast-ack and commit idempotently without direct posting; Plaid, Stripe and Gusto use controlled reconciliation or maker-checker drafts; HubSpot uses immutable identity links and maker-checker non-posting proposals (`lib/webhook-verification.js`; `lib/webhook-worker.js`; `lib/crm-handoff.js`; `test/webhook-worker-crash.test.js`; `server.js:webhook`)                   | Payroll/expense signatures remain Folio-specific and provider-global; native Gusto and HubSpot webhook verification are incomplete; no live deployment process-kill evidence            | Implement remaining provider-native verification and connection binding; exercise worker kill/restart in credentialed staging                                 | Invalid signatures, stale timestamps, duplicate/account mismatch, stale queue and dead-letter alerts                            | Medium until native integration | High            | high     |
| TM-005    | Compromised provider/connector credential         | Live connector enabled                                    | Returns reordered, duplicated, omitted or malicious records or steals secret                           | Silent accounting incompleteness or external data access       | Provider credentials, cursors, normalized records | One-time tenant/user/connection-bound OAuth state, AES-256-GCM token vault, refresh lease, provider account binding, safe errors, fixed HTTPS hosts, schemas, hashes, bounded retries and staging/dead letters (`lib/provider-oauth.js`; `lib/integrations.js`; `lib/provider-adapters.js`)                                                                                                                                                                                                                                                       | Provider sandbox/least-privilege certification and live schema/failure evidence remain absent; cursor correctness depends on provider semantics                                         | Complete provider sandbox/chaos tests, Plaid Link, native webhook coverage and source count/total reconciliation before committing cursors                    | Sync freshness, count/value deltas, cursor regressions, auth failures and dead-letter SLA alerts                                | Medium                          | High            | high     |
...
| TM-009    | Remote/authenticated DoS actor or load spike      | Can send requests or trigger heavy operations             | Saturates single Node process, SQLite writer, disk or report generation                                | Whole-service outage and missed accounting operations          | Availability, recovery objectives                 | Request/header timeouts, body limits, bounded metric cardinality, session-bound per-user rate limits, global/per-tenant concurrency bulkheads, durable report/provider/import jobs with claims, leases, retries/dead letters, source/artifact expiry and queue alerts, one-writer topology, readiness/alerts, and a CI-gated 10,000-row import/apply/fuzzy-reindex capacity case (`lib/admission-control.js`; `lib/background-worker.js`; `server.js:createFolioServer`; `scripts/load-benchmark.js`; `compose.production.yml`; `ops/alerts.yml`) | Limits and the queue use the single platform process/database topology; no upstream WAF, distributed limiter, tenant storage quota, or deployed multi-tenant soak evidence              | Add edge/WAF anonymous limits, a shared limiter/queue backend for horizontal scale, storage quotas, and deployed capacity/soak evidence                       | Admission rejections, background queue age/dead letters, event-loop lag, SQLite busy time, tenant-level latency and disk alerts | Medium                          | High            | high     |
