...
| TM-004    | Remote attacker with leaked/shared webhook secret | Provider secret is known or provider channel compromised  | Replays or forges HMAC payload for a chosen tenant slug                                                | Unauthorized cash, invoices, payroll or expense journals       | Ledgers, webhook audit, cash/AR                   | Stripe authenticates exact raw body/timestamp with connection-scoped secret and account binding; production rejects slug-only endpoints; tenant inbox atomically commits replay key, payload hash and result; supported native events are staged without direct posting (`lib/webhook-verification.js`; `lib/provider-adapters.js:stripeWebhookPage`; `lib/db.js:applyExternalEvent`; `server.js:webhook`) | Payroll/expense signatures remain Folio-specific and provider-global; request processing is synchronous; staged-record-to-accounting mapping/application is incomplete | Implement remaining provider-native verification and connection binding; add controlled mapping/application and a fast-ack queue/worker while preserving tenant-transactional application | Invalid signatures, stale timestamps, duplicate/event-account mismatch and webhook posting alerts | Medium until native integration | High            | high     |
