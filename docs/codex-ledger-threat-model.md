...
| TM-009    | Remote/authenticated DoS actor or load spike      | Can send requests or trigger heavy operations             | Saturates single Node process, SQLite writer, disk or report generation                                | Whole-service outage and missed accounting operations          | Availability, recovery objectives                 | Request/header timeouts, body limits, bounded metric cardinality, session-bound per-user rate limits, global/per-tenant concurrency bulkheads, stricter expensive-route pools, retry guidance, rejection metrics/alerting, one-writer topology, readiness/alerts, and a CI-gated 10,000-row import/apply/fuzzy-reindex capacity case (`lib/admission-control.js`; `server.js:createFolioServer`; `scripts/load-benchmark.js`; `compose.production.yml`; `ops/alerts.yml`)                                                                                  | Limits are process-local and reset on restart; no upstream WAF, distributed limiter, durable report/sync queue, tenant storage quota, or deployed multi-tenant soak evidence            | Add edge/WAF anonymous limits, a shared limiter for horizontal scale, durable job queues for reports/sync, storage quotas, and deployed capacity/soak evidence        | Admission rejections, event-loop lag, SQLite busy time, queue depth, tenant-level latency and disk alerts | Medium                          | High            | high     |
