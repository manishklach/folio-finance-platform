...
| TM-004    | Remote attacker with leaked/shared webhook secret | Provider secret is known or provider channel compromised  | Replays or forges HMAC payload for a chosen tenant slug                                                | Unauthorized cash, invoices, payroll or expense journals       | Ledgers, webhook audit, cash/AR                   | Stripe authenticates the exact raw body and signed timestamp with a bounded tolerance and supports rotation signatures; all sources have constant-time checks, body caps and event-ID replay records (`lib/webhook-verification.js`; `server.js:webhook`) | Payroll/expense signatures remain Folio-specific; secrets are provider-global, not connection/tenant scoped; no durable queue before processing      | Implement remaining provider-native verification; per-connection secret; durable inbox transaction; bind event account to connection/tenant                               | Invalid signatures, stale timestamps, duplicate/event-account mismatch and webhook posting alerts | Medium until native integration | High            | high     |
