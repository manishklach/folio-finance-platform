...
function render() {
  document
    ...
    .forEach((b) => b.classList.toggle("active", b.dataset.view === state.currentView));
  (
    ({
...
function kpi(label, value, icon, note, neutral = false) {
  return `<article class="card kpi"><div class="kpi-top"><span>${label}</span><span class="kpi-icon">${icon}</span></div><strong>${value}</strong><span class="delta ${neutral ? "neutral" : ""}">${note}</span></article>`;
}
...
function renderJournals() {
  ...
  document
    ...
    .forEach((b) => b.addEventListener("click", () => postEntry(b.dataset.post)));
}
...
function renderRevenue() {
  ...
  <div class="grid"><article class="card panel"><div class="panel-head"><div><h3>Revenue waterfall</h3><p>Scheduled and recognized by performance period</p></div><button id="recognize-revenue">Recognize through Aug 22 →</button></div><div class="waterfall">${s.waterfall
    .slice(0, 14)
    .map(
      (r) =>
        `<div class="waterfall-row"><span>${r.period}</span><div class="waterfall-track"><i class="waterfall-fill" style="display:block;width:${(r.recognized_cents / r.scheduled_cents) * 100}%"></i></div><strong>${money(r.scheduled_cents)}</strong></div>`,
    )
    .join("")}</div></article>
...
async function runRecognition() {
  try {
    const result = await api("/api/revenue/recognize", {
      method: "POST",
      body: JSON.stringify({ as_of: "2026-08-22" }),
    });
    await load();
    navigate("revenue");
    toast(`${result.recognized_schedules} revenue schedules posted with ASC 606 audit lineage.`);
  } catch (error) {
...
function renderReceivables() {
  ...
  const ar = state.saas.receivables,
    a = ar.aging,
    r = ar.reconciliation,
    overdue = a.total_cents - a.current_cents;
  const agingRows = [
    ["Current", a.current_cents],
    ["1–30 days", a.days_1_30_cents],
    ["31–60 days", a.days_31_60_cents],
    ["61–90 days", a.days_61_90_cents],
    ["90+ days", a.days_90_plus_cents],
  ];
  ...
  document.querySelectorAll("[data-ar-open]").forEach((button) =>
    button.addEventListener("click", () =>
      openArForm(button.dataset.arOpen, {
        invoiceId: Number(button.dataset.invoice) || null,
        paymentId: Number(button.dataset.payment) || null,
        disputeId: Number(button.dataset.dispute) || null,
      }),
    ),
  );
  document
    .querySelectorAll("[data-void-invoice]")
    .forEach((button) =>
      button.addEventListener("click", () =>
        voidArObject("invoice", Number(button.dataset.voidInvoice)),
      ),
    );
  document
    .querySelectorAll("[data-void-payment]")
    .forEach((button) =>
      button.addEventListener("click", () =>
        voidArObject("payment", Number(button.dataset.voidPayment)),
      ),
    );
  document
    .querySelectorAll("[data-complete-collection]")
    .forEach((button) =>
      button.addEventListener("click", () =>
        completeCollection(Number(button.dataset.completeCollection)),
      ),
    );
}
...
function renderMetrics() {
  ...
  const cards = [
    ["ARR", money(m.arr_cents), "MRR " + money(m.mrr_cents)],
    ["Net revenue retention", pct(m.nrr), `GRR ${pct(m.grr)}`],
    ["Gross margin", pct(m.gross_margin), "After cloud cost of revenue"],
    [
      "Rule of 40",
      `${m.rule_of_40.toFixed(1)}`,
      `${pct(m.growth_rate)} growth + ${pct(m.operating_margin)} margin`,
    ],
    ["Bookings", money(m.bookings_cents), `Billings ${money(m.billings_cents)}`],
    ["Average contract value", money(m.acv_cents), "Signed contract consideration"],
    ["CAC", money(m.cac_cents), `LTV ${money(m.ltv_cents)}`],
    ["Burn multiple", m.burn_multiple.toFixed(2), `Magic number ${m.magic_number.toFixed(2)}`],
  ];
...
function renderReports() {
  ...
  const revenue = state.accounts.filter((a) => a.type === "revenue"),
    expenses = state.accounts.filter((a) => a.type === "expense"),
    assets = state.accounts.filter((a) => a.type === "asset"),
    liabilities = state.accounts.filter((a) => a.type === "liability"),
    equity = state.accounts.filter((a) => a.type === "equity");
  const sum = (list) => list.reduce((s, a) => s + a.balance_cents, 0);
  const earnings = sum(revenue) - sum(expenses),
    balanceCheck = sum(assets) - sum(liabilities) - sum(equity) - earnings;
...
function reportTable(accounts, label, total) {
  return `<table class="table"><tbody>${accounts.map((a) => `<tr><td><span class="type-dot type-${a.type}"></span>${a.name}</td><td class="amount">${money(a.balance_cents)}</td></tr>`).join("")}<tr class="report-total"><td>${label}</td><td class="amount">${money(total)}</td></tr></tbody></table>`;
}
...
function renderOperations() {
  ...
  const s = state.saas,
    commissionTotal = s.commissions.reduce((n, c) => n + c.amount_cents, 0),
    capitalized = s.software_projects.reduce((n, p) => n + p.capitalized_cents, 0),
    expensed = s.software_projects.reduce((n, p) => n + p.expensed_cents, 0);
...
function renderEntities() {
  ...
  document
    .querySelector("#run-fx")
    .addEventListener("click", () => runGlobal("/api/fx/revalue", "FX revaluation posted"));
  document
    .querySelector("#run-elimination")
    .addEventListener("click", () =>
      runGlobal("/api/consolidation/eliminate", "Intercompany elimination posted"),
    );
}
async function runGlobal(path, message) {
  try {
    await api(path, { method: "POST", body: JSON.stringify({ as_of: "2026-08-22" }) });
    await load();
    navigate("entities");
    toast(message);
  } catch (error) {
...
function openArForm(kind, { invoiceId = null, paymentId = null, disputeId = null } = {}) {
  ...
  if (kind === "payment") {
    ...
    title = `Refund ${payment?.payment_number}`;
    const rows = ar.invoices.filter(
      (i) => i.customer_id === payment?.customer_id && i.applied_cents > i.refund_cents,
    );
    body = `<input type="hidden" name="payment_id" value="${paymentId}">${field("Related invoice", `<select name="invoice_id">${invoiceOptions(rows)}</select>`)}<div class="form-pair">${field("Refund amount ($)", amount(payment?.unapplied_cents ? payment.unapplied_cents / 100 : ""))}${field("Refund date", `<input type="date" name="date" value="${today}" required>`)}</div>${field("Refund number", `<input name="number" value="RF-${Date.now().toString().slice(-6)}" required>`)}${field("Reason", `<textarea name="reason" rows="3" required></textarea>`)}`;
  } else if (kind === "dispute") {
    title = `Open dispute for ${invoice?.invoice_number}`;
    body = `<input type="hidden" name="invoice_id" value="${invoiceId}"><div class="form-pair">${field("Disputed amount ($)", amount((invoice?.balance_cents - invoice?.disputed_cents) / 100))}${field("Opened date", `<input type="date" name="date" value="${today}" required>`)}</div>${field("Customer's reason", `<textarea name="reason" rows="3" required></textarea>`)}`;
  } else if (kind === "resolve") {
    title = "Resolve invoice dispute";
    body = `<input type="hidden" name="dispute_id" value="${disputeId}">${field("Resolution", `<textarea name="reason" rows="3" required placeholder="Outcome and supporting evidence"></textarea>`)}<div class="form-pair">${field("Resolved date", `<input type="date" name="date" value="${today}" required>`)}${field("Outcome", `<select name="status"><option value="resolved">Resolved</option><option value="withdrawn">Withdrawn</option></select>`)}</div>`;
  ...
  document.querySelector("#ar-form-body").innerHTML =
    `<form id="ar-activity-form" class="contract-form" data-kind="${kind}">${body}<button class="primary wide" type="submit">Post and save</button></form>`;
...
async function saveArActivity(event) {
  ...
  const form = event.currentTarget,
    ...
    cents = Math.round(Number(f.get("amount") || 0) * 100),
    date = f.get("date");
  let path, payload;
  if (kind === "payment") {
    path = "/api/receivables/payments";
    const invoiceId = Number(f.get("invoice_id"));
    payload = {
      customer_id: Number(f.get("customer_id")),
      payment_number: f.get("number"),
      payment_date: date,
      amount_cents: cents,
      method: f.get("method"),
      reference: f.get("reference"),
      applications: invoiceId ? [{ invoice_id: invoiceId, amount_cents: cents }] : [],
    };
  } else if (kind === "application") {
    path = "/api/receivables/applications";
    payload = {
      payment_id: Number(f.get("payment_id")),
    ...
    path = "/api/invoices";
    payload = {
      contract_id: Number(f.get("contract_id")),
    ...
    path = "/api/receivables/credits";
    payload = {
      invoice_id: Number(f.get("invoice_id")),
      credit_number: f.get("number"),
      credit_date: date,
      amount_cents: cents,
      reason: f.get("reason"),
    };
  } else if (kind === "writeoff") {
    path = "/api/receivables/write-offs";
    payload = {
      invoice_id: Number(f.get("invoice_id")),
      write_off_date: date,
      amount_cents: cents,
    ...
    path = "/api/receivables/refunds";
    payload = {
      payment_id: Number(f.get("payment_id")),
      invoice_id: Number(f.get("invoice_id")) || null,
      refund_number: f.get("number"),
      refund_date: date,
      amount_cents: cents,
      reason: f.get("reason"),
    };
  } else if (kind === "dispute") {
    path = "/api/receivables/disputes";
    payload = {
      invoice_id: Number(f.get("invoice_id")),
      opened_date: date,
      amount_cents: cents,
    ...
    path = "/api/receivables/disputes/resolve";
    payload = {
      dispute_id: Number(f.get("dispute_id")),
      resolved_date: date,
      resolution: f.get("reason"),
      status: f.get("status"),
    };
  } else {
    path = "/api/receivables/collections";
    payload = {
      customer_id: Number(f.get("customer_id")),
      invoice_id: Number(f.get("invoice_id")) || null,
      activity_date: date,
      ...
      notes: f.get("reason"),
      next_action_date: f.get("next_action_date") || null,
    };
  }
  ...
  try {
    ...
    document.querySelector("#ar-overlay").hidden = true;
    await load();
    navigate("receivables");
    toast("Receivables activity posted with journal and audit lineage.");
  } catch (error) {
    toast(error.message, true);
    button.disabled = false;
    button.textContent = "Post and save";
  }
}
async function voidArObject(kind, id) {
  if (
    !confirm(`Void this ${kind}? Folio will post a reversing journal and retain the audit trail.`)
  )
    return;
  try {
    const date = state.saas.receivables.as_of;
    await api(
      kind === "invoice" ? `/api/invoices/${id}/void` : `/api/receivables/payments/${id}/void`,
      {
        method: "POST",
        body: JSON.stringify(
          kind === "invoice" ? { void_date: date, reason: "Voided by user" } : { void_date: date },
        ),
      },
    );
    await load();
    navigate("receivables");
    toast(`${kind[0].toUpperCase() + kind.slice(1)} voided with a reversing journal.`);
  } catch (error) {
    toast(error.message, true);
  }
}
async function completeCollection(id) {
  try {
    await api("/api/receivables/collections/complete", {
      method: "POST",
      body: JSON.stringify({ activity_id: id }),
    });
    await load();
    navigate("receivables");
    toast("Collection action completed.");
  } catch (error) {
    toast(error.message, true);
  }
}
...
function bindLinks() {
  document
    ...
    .forEach((b) => b.addEventListener("click", () => navigate(b.dataset.viewLink)));
}
...
document.querySelector("#new-entry").addEventListener("click", () => {
  ...
  document.querySelector("#ai-description").focus();
});
...
document
  ...
  .forEach((b) =>
    b.addEventListener(
      "click",
      () => (document.querySelector("#ai-description").value = b.textContent),
    ),
  );
document.querySelector("#generate-draft").addEventListener("click", generateDraft);

const contractOverlay = document.querySelector("#contract-overlay"),
  contractForm = document.querySelector("#contract-form");
document
  .querySelector("[data-contract-close]")
  .addEventListener("click", () => (contractOverlay.hidden = true));
contractOverlay.addEventListener("click", (e) => {
  if (e.target === contractOverlay) contractOverlay.hidden = true;
});
contractForm.addEventListener("submit", saveContract);
const arOverlay = document.querySelector("#ar-overlay");
document
  .querySelector("[data-ar-close]")
  .addEventListener("click", () => (arOverlay.hidden = true));
arOverlay.addEventListener("click", (e) => {
  if (e.target === arOverlay) arOverlay.hidden = true;
});

function openContractForm() {
  contractForm.elements.customer_id.innerHTML = state.saas.customers
    .map((c) => `<option value="${c.id}">${escapeHtml(c.name)}</option>`)
    .join("");
  contractForm.elements.contract_number.value = `NEW-${Date.now().toString().slice(-6)}`;
  contractForm.elements.start_date.value = "2026-09-01";
  contractForm.elements.end_date.value = "2027-08-31";
  contractOverlay.hidden = false;
}

async function saveContract(event) {
  event.preventDefault();
  const f = new FormData(contractForm),
    cents = (name) => Math.round(Number(f.get(name) || 0) * 100),
    price = cents("price");
  const payload = {
    customer_id: Number(f.get("customer_id")),
    entity_id: 1,
    ...
    variable_consideration_cents: cents("variable"),
    constraint_percent: Number(f.get("constraint")),
    billing_model: f.get("method") === "usage" ? "usage" : "subscription",
    commission_cents: cents("commission"),
    obligations: [
      {
        product_id: state.saas.products[0]?.id,
        description: f.get("obligation"),
        ssp_cents: cents("ssp"),
        recognition_method: f.get("method"),
        total_units: f.get("method") === "usage" ? 100000 : 0,
      },
    ],
  };
  try {
    const contract = await api("/api/contracts", { method: "POST", body: JSON.stringify(payload) });
    const invoice = cents("invoice");
    if (invoice)
    ...
    await load();
    navigate("revenue");
    toast("Contract allocated, schedules created, and billing posted.");
  } catch (error) {
    toast(error.message, true);
  }
}
...
async function generateDraft() {
  const button = document.querySelector("#generate-draft"),
    description = document.querySelector("#ai-description").value;
  button.disabled = true;
  button.textContent = "Working through the accounting…";
  try {
    state.proposal = await api("/api/ai/draft", {
      method: "POST",
...
async function saveDraft() {
  ...
  try {
    ...
    state.proposal = null;
    document.querySelector("#draft-result").innerHTML = "";
    document.querySelector("#ai-description").value = "";
    await load();
    navigate("journals");
    toast("Draft saved. It has not been posted.");
  } catch (error) {
    toast(error.message, true);
  }
}
async function postEntry(id) {
  ...
  try {
    ...
    await load();
    navigate("journals");
    toast("Entry posted and sealed with an integrity hash.");
  } catch (error) {
    toast(error.message, true);
  }
}
function toast(message, isError = false) {
  ...
  el.style.background = isError ? "#8c3030" : "#17211b";
  el.hidden = false;
