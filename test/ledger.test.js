...
test("rejects an unbalanced journal", () => {
  const result = validateJournal({ date: "2026-08-22", memo: "Bad entry", lines: [
...
test("posts a balanced draft and records its integrity hash", () => {
  ...
  const draft = ledger.createDraft({ date: "2026-08-22", memo: "Test cash sale", lines: [
...
test("prevents posting into a closed period", () => {
  const ledger = createLedger(":memory:", { seed: true });
  const accounts = Object.fromEntries(ledger.getAccounts().map(a => [a.code, a.id]));
  const draft = ledger.createDraft({ date: "2026-08-22", memo: "Late expense", lines: [
...
test("posted trial balance remains balanced", () => {
  ...
  const debitNormal = rows.filter(a => ["asset", "expense"].includes(a.type)).reduce((sum, a) => sum + a.balance_cents, 0);
  const creditNormal = rows.filter(a => ["liability", "equity", "revenue"].includes(a.type)).reduce((sum, a) => sum + a.balance_cents, 0);
