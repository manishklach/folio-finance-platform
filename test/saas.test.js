...
test("ASC 606 allocates constrained consideration by relative SSP exactly", () => {
  ...
  assert.equal(
    contract.obligations.reduce((s, o) => s + o.allocated_price_cents, 0),
    15_500_000,
  );
  const scheduled = ledger
    .revenueSchedules()
    .filter((s) => contract.obligations.some((o) => o.id === s.obligation_id))
    .reduce((s, x) => s + x.amount_cents, 0);
  const usageAllocation = contract.obligations.find(
    (o) => o.recognition_method === "usage",
...
test("billing and recognition remain separate and produce an auditable waterfall", () => {
  ...
  assert.equal(
    ledger.revenueWaterfall().some((row) => row.recognized_cents > 0),
    true,
  );
  assert.equal(
    ledger
      ...
      .reduce(
        (s, a) =>
          s + (a.type === "asset" || a.type === "expense" ? a.balance_cents : -a.balance_cents),
        0,
      ),
    0,
  );
...
test("FX revaluation and intercompany elimination create balanced posted entries", () => {
  const ledger = createLedger(":memory:", { seed: true });
  ledger.db
    .prepare("INSERT INTO fx_rates(rate_date,currency,usd_rate) VALUES('2026-08-31','EUR',1.20)")
    .run();
...
test("usage, milestone, extension, cancellation and renewal workflows update schedules", () => {
  ...
  const milestone = ledger.createContract({
    customer_id: customer.id,
    entity_id: 1,
    contract_number: "MILE-1",
    signed_date: "2026-08-01",
    start_date: "2026-08-01",
    end_date: "2026-12-31",
    transaction_price_cents: 1_000_000,
    obligations: [
      {
        product_id: product.id,
        description: "Deployment milestone",
        ssp_cents: 1_000_000,
        recognition_method: "milestone",
      },
    ],
  });
  const progress = ledger.updateMilestone({
    obligation_id: milestone.obligations[0].id,
    event_date: "2026-08-20",
    progress: 25,
  });
  assert.equal(progress.amount_cents, 250_000);
  const subscription = ledger.createContract({
    customer_id: customer.id,
    entity_id: 1,
    contract_number: "EXT-1",
    signed_date: "2026-08-01",
    start_date: "2026-09-01",
    end_date: "2026-11-30",
    transaction_price_cents: 900_000,
    obligations: [
      {
        product_id: product.id,
  ...
  ledger.modifyContract({
    ...
    kind: "extension",
    description: "Extend two months",
    new_end_date: "2027-01-31",
  });
  const extendedSchedules = ledger
    .revenueSchedules()
    .filter((s) => s.contract_number === "EXT-1" && s.status === "pending");
  assert.equal(extendedSchedules.length, 5);
  assert.equal(
    extendedSchedules.reduce((s, x) => s + x.amount_cents, 0),
    900_000,
  );
  const renewal = ledger.createContract({
    customer_id: customer.id,
    entity_id: 1,
    renewal_of: subscription.id,
    contract_number: "EXT-2",
    signed_date: "2027-01-15",
    start_date: "2027-02-01",
    end_date: "2028-01-31",
    transaction_price_cents: 1_200_000,
    obligations: [
      {
        product_id: product.id,
        description: "Renewal",
  ...
  ledger.modifyContract({
    ...
    kind: "cancellation",
    description: "Customer terminated",
  });
  assert.equal(ledger.getContract(subscription.id).status, "cancelled");
  assert.equal(
    ledger.revenueSchedules().filter((s) => s.contract_number === "EXT-1" && s.status === "pending")
      .length,
    1,
  );
  ...
  assert.equal(
    ledger
      .revenueSchedules()
      .filter((s) => s.contract_number === "EXT-1" && s.status === "recognized").length,
    1,
  );
  ledger.close();
});
...
test("receivables supports partial payments, credits, write-offs, refunds, disputes and collections", () => {
  ...
  const payment = ledger.recordPayment({
    ...
    method: "ach",
    reference: "BANK-991",
    applications: [{ invoice_id: 1, amount_cents: 5_000_000 }],
  });
  ...
  assert.throws(
    () =>
      ledger.recordPayment({
        ...
        applications: [{ invoice_id: 1, amount_cents: 100_000 }],
      }),
    /same customer/,
  );
  assert.throws(
  ...
  assert.equal(
    ledger.listJournals().length,
    journalsAfterPayment,
    "invalid receipts do not leave orphan journals",
  );
  ...
  ledger.createCreditMemo({
    invoice_id: 1,
    credit_number: "CM-TEST-1",
    credit_date: "2026-08-12",
    amount_cents: 1_000_000,
    reason: "Service-level credit",
  });
  ledger.writeOffInvoice({
    invoice_id: 1,
    write_off_date: "2026-08-13",
    amount_cents: 500_000,
    reason: "Approved immaterial balance",
  });
  ledger.openDispute({
    invoice_id: 1,
    opened_date: "2026-08-14",
    amount_cents: 2_000_000,
    reason: "Customer contests usage",
  });
  ledger.addCollectionActivity({
    customer_id: 1,
    invoice_id: 1,
    activity_date: "2026-08-15",
    activity_type: "call",
    notes: "CFO promised supporting detail",
  ...
  assert.equal(
    ar.reconciliation.ar_difference_cents,
    1_000_000,
    "direct EU GL receivable is correctly surfaced as an exception",
  );
  assert.equal(ar.reconciliation.unapplied_difference_cents, 0);

  ledger.refundPayment({
    payment_id: payment.id,
    invoice_id: 1,
  ...
  const dispute = ledger
    .receivables("2026-08-22")
    .disputes.find((d) => d.invoice_id === 1 && d.status === "open");
  ledger.resolveDispute({
    dispute_id: dispute.id,
    resolved_date: "2026-08-21",
    resolution: "Usage evidence accepted",
  });
...
test("payment and invoice voids reverse subledger and general-ledger effects", () => {
  ...
  const payment = ledger.recordPayment({
    customer_id: 2,
    payment_number: "PAY-VOID-1",
    payment_date: "2026-08-10",
    amount_cents: 2_000_000,
    applications: [{ invoice_id: 2, amount_cents: 1_000_000 }],
  });
  assert.equal(
    ledger.listInvoices("2026-08-22").find((i) => i.id === 2).balance_cents,
    before - 1_000_000,
  );
  assert.equal(
    ledger.voidPayment({ payment_id: payment.id, void_date: "2026-08-11" }).status,
    "void",
  );
  ...
  const voided = ledger.voidInvoice({
    invoice_id: invoice.id,
    void_date: "2026-08-13",
