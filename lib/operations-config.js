...
export function validateOperationsConfig(environment = process.env) {
  ...
  const providerTokenKeyPath = requiredSecretFile(
    environment,
    "PROVIDER_TOKEN_ENCRYPTION_KEY_FILE",
  );
  normalizeKey(readFileSync(providerTokenKeyPath, "utf8"));
  if (!/^[A-Za-z0-9._-]{3,80}$/.test(environment.PROVIDER_TOKEN_KEY_ID || ""))
    throw new Error("PROVIDER_TOKEN_KEY_ID must be a non-secret rotation identifier");
