...
export function validateJournal(entry, accountIds = null) {
  ...
  for (const [index, line] of (entry?.lines || []).entries()) {
    ...
    if (debit > 0 === credit > 0)
      errors.push(`Line ${index + 1} must contain either a debit or a credit`);
    if (!Number.isInteger(Number(line.account_id)))
      errors.push(`Line ${index + 1} needs an account`);
    if (accountIds && !accountIds.has(Number(line.account_id)))
      errors.push(`Line ${index + 1} references an unknown account`);
    debits += debit;
    credits += credit;
  }
  if (debits === 0 || credits === 0) errors.push("The entry must have non-zero debits and credits");
  if (debits !== credits)
    errors.push(`Entry is out of balance by ${fromCents(Math.abs(debits - credits)).toFixed(2)}`);
...
export function canonicalJournalHash(entry, lines) {
  const canonical = JSON.stringify({
    ...
    memo: entry.memo,
    lines: [...lines]
      .map(({ account_id, debit_cents, credit_cents, description }) => ({
        account_id,
        debit_cents,
        credit_cents,
