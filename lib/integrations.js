...
export function createIntegrationsRepository(
  ...
  function configureIntegration(input) {
    ...
    if (
      ...
      !["stripe", "gusto", "hubspot"].includes(provider)
    )
  ...
  function bindIntegrationAccount(input) {
    const connection = integrationConnection(input.connection_id);
    const externalAccountId = externalIdSchema.parse(input.external_account_id);
    if (connection.external_account_id && connection.external_account_id !== externalAccountId)
      throw bad("OAuth account does not match the configured provider account", 409);
    try {
      db.prepare(
        "UPDATE integration_connections SET external_account_id=?,status='active',last_error=NULL,consecutive_failures=0,updated_at=CURRENT_TIMESTAMP WHERE id=?",
      ).run(externalAccountId, connection.id);
    } catch (error) {
      if (/UNIQUE constraint failed/i.test(String(error?.message)))
        throw bad("Provider account is already connected", 409);
      throw error;
    }
    return integrationConnection(connection.id);
  }
