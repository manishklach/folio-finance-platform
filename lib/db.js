...
import {
  createStripeReconciliationRepository,
  migrateStripeReconciliation,
} from "./stripe-reconciliation.js";
import { createPayrollRepository, migratePayroll } from "./payroll.js";
import { createCrmHandoffRepository, migrateCrmHandoff } from "./crm-handoff.js";
...
export function createLedger(dbPath = DEFAULT_DB_PATH, { seed = true, orgId = "legacy" } = {}) {
  ...
  try {
    ...
    migratePayroll(db);
    migrateStripeReconciliation(db);
    migrateGaap(db);
    migrateCrmHandoff(db);
    ...
    const payroll = createPayrollRepository(db, repository);
    const crmHandoff = createCrmHandoffRepository(db, saas);
    const stripeReconciliation = createStripeReconciliationRepository(db);
...
function makeRepository(db) {
  ...
  function postJournal(id, actor = currentActor()) {
    ...
    if (
      ["provider_integration", "payroll_integration", "payroll_settlement"].includes(entry.source)
    ) {
      const application = db
        .prepare("SELECT approved_by FROM integration_record_applications WHERE journal_entry_id=?")
        .get(id);
      const payrollApproval = db
        .prepare("SELECT approved_by FROM payroll_runs WHERE accounting_journal_id=?")
        .get(id)?.approved_by;
      const settlementPreparer = db
        .prepare("SELECT prepared_by FROM payroll_settlements WHERE journal_entry_id=?")
        .get(id)?.prepared_by;
      if ([application?.approved_by, payrollApproval, settlementPreparer].includes(actor))
        throw Object.assign(
          new Error("Provider-controlled drafts must be posted by a different approver"),
          { statusCode: 409 },
        );
    }
    const period = db
...
function seedAdditionalAccounts(db) {
  const accounts = [
    ...
    ["2160", "Net payroll payable", "liability"],
    ["2165", "Employee reimbursements payable", "liability"],
    ["2170", "Payroll taxes payable", "liability"],
    ["2180", "Sales tax and VAT payable", "liability"],
    ["2195", "Benefit deductions and contributions payable", "liability"],
    ["2196", "Other payroll deductions payable", "liability"],
    ...
    ["5210", "Employer payroll tax expense", "expense"],
    ["5220", "Employee benefit expense", "expense"],
    ["5230", "Employee reimbursement expense", "expense"],
