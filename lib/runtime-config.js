...
import { admissionConfig } from "./admission-control.js";

export function validateProductionConfig(environment = process.env) {
  admissionConfig(environment);
  ...
  const stripeTolerance = Number(environment.STRIPE_WEBHOOK_TOLERANCE_SECONDS || 300);
  if (!Number.isInteger(stripeTolerance) || stripeTolerance < 1 || stripeTolerance > 900)
    throw new Error("STRIPE_WEBHOOK_TOLERANCE_SECONDS must be 1 through 900");
