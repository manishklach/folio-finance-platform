...
export function createSaasRepository(db, ledger) {
  ...
  function createInvoice(input) {
    ...
    requireAmount(input.amount_cents,"Invoice amount");
    if (!input.invoice_number?.trim() || !input.invoice_date) throw bad("Invoice number and date are required");
    if(db.prepare("SELECT id FROM invoices WHERE invoice_number=?").get(input.invoice_number.trim()))throw bad("Invoice number already exists",409);
    ...
    const result = db.prepare("INSERT INTO invoices(contract_id,invoice_number,invoice_date,due_date,amount_cents,journal_entry_id) VALUES(?,?,?,?,?,?)")
      .run(input.contract_id,input.invoice_number.trim(),input.invoice_date,input.due_date||addDays(input.invoice_date,30),input.amount_cents,posted.id);
  ...
  function decorateInvoice(row,asOf) {
    if(!row) return null;
    const balance=Math.max(0,row.amount_cents-row.applied_cents-row.credit_cents-row.write_off_cents+row.refund_cents);
    const daysPastDue=row.due_date?Math.max(0,daysBetween(row.due_date,asOf)):0;
    let status=row.status;
    if(status!=="void") status=row.disputed_cents>0?"disputed":balance===0?"paid":row.applied_cents+row.credit_cents+row.write_off_cents>0?"partially_paid":daysPastDue>0?"overdue":"open";
    return {...row,balance_cents:balance,days_past_due:daysPastDue,status};
  }
  function invoiceRecord(id,asOf=new Date().toISOString().slice(0,10)){return decorateInvoice(db.prepare(`${invoiceBaseSql} WHERE i.id=? AND i.invoice_date<=?`).get(asOf,asOf,asOf,asOf,asOf,id,asOf),asOf);}
  function listInvoices(asOf=new Date().toISOString().slice(0,10)){return db.prepare(`${invoiceBaseSql} WHERE i.invoice_date<=? ORDER BY i.due_date,i.id`).all(asOf,asOf,asOf,asOf,asOf,asOf).map(row=>decorateInvoice(row,asOf));}
  function paymentRecord(id){return db.prepare(`SELECT p.*,cu.name customer_name,
    ...
    FROM customer_payments p JOIN customers cu ON cu.id=p.customer_id WHERE p.id=?`).get(id);}
  function listPayments(asOf=new Date().toISOString().slice(0,10)){return db.prepare(`SELECT p.*,cu.name customer_name,
    ...
    FROM customer_payments p JOIN customers cu ON cu.id=p.customer_id WHERE p.payment_date<=? ORDER BY p.payment_date DESC,p.id DESC`).all(asOf,asOf,asOf).map(p=>({...p,unapplied_cents:Math.max(0,p.amount_cents-p.applied_cents-p.refunded_cents)}));}

  function recordPayment(input) {
    const customer=db.prepare("SELECT * FROM customers WHERE id=?").get(input.customer_id); if(!customer)throw bad("Customer not found",404);
    ...
    if(db.prepare("SELECT id FROM customer_payments WHERE payment_number=?").get(number))throw bad("Payment number already exists",409);
    let applicationTotal=0;
    for(const application of input.applications||[]){requireAmount(application.amount_cents,"Application amount");const invoice=invoiceRecord(application.invoice_id,input.payment_date);if(!invoice||invoice.status==="void")throw bad("Open invoice not found",404);if(invoice.customer_id!==input.customer_id)throw bad("Payment and invoice must belong to the same customer");if(application.amount_cents>invoice.balance_cents)throw bad("Application exceeds invoice balance");applicationTotal+=application.amount_cents;}
    if(applicationTotal>input.amount_cents)throw bad("Applications exceed payment amount");
    const entityId=input.entity_id||1;
    const draft=ledger.createDraft({date:input.payment_date,memo:`Customer payment ${number} — ${customer.name}`,source:"cash_receipts",entity_id:entityId,lines:[
      {account_id:accountId("1000"),debit_cents:input.amount_cents,credit_cents:0},{account_id:accountId("2150"),debit_cents:0,credit_cents:input.amount_cents}
    ]},"receivables.engine");
    const journal=ledger.postJournal(draft.id,"receivables.engine");
    const result=db.prepare("INSERT INTO customer_payments(customer_id,entity_id,payment_number,payment_date,amount_cents,method,reference,journal_entry_id) VALUES(?,?,?,?,?,?,?,?)")
      .run(input.customer_id,entityId,number,input.payment_date,input.amount_cents,input.method||"ach",input.reference||null,journal.id);
    const paymentId=Number(result.lastInsertRowid);
    audit(db,"payment",paymentId,"payment_received","receivables.engine",{amount_cents:input.amount_cents,journal_entry_id:journal.id});
    for(const application of input.applications||[]) applyPayment({payment_id:paymentId,invoice_id:application.invoice_id,amount_cents:application.amount_cents,applied_date:input.payment_date});
    return paymentRecord(paymentId);
  }

  function applyPayment(input) {
    const payment=paymentRecord(input.payment_id); if(!payment||payment.status!=="received")throw bad("Active payment not found",404);
    const invoice=invoiceRecord(input.invoice_id,input.applied_date); if(!invoice||invoice.status==="void")throw bad("Open invoice not found",404);
    if(invoice.customer_id!==payment.customer_id)throw bad("Payment and invoice must belong to the same customer");
    ...
    if(input.amount_cents>invoice.balance_cents)throw bad("Application exceeds invoice balance");
    const date=input.applied_date||payment.payment_date;
    const draft=ledger.createDraft({date,memo:`Apply ${payment.payment_number} to ${invoice.invoice_number}`,source:"cash_application",entity_id:payment.entity_id,lines:[
      {account_id:accountId("2150"),debit_cents:input.amount_cents,credit_cents:0},{account_id:accountId("1100"),debit_cents:0,credit_cents:input.amount_cents}
    ]},"receivables.engine");
    const journal=ledger.postJournal(draft.id,"receivables.engine");
    const result=db.prepare("INSERT INTO payment_applications(payment_id,invoice_id,amount_cents,applied_date,journal_entry_id) VALUES(?,?,?,?,?)").run(payment.id,invoice.id,input.amount_cents,date,journal.id);
    audit(db,"payment_application",Number(result.lastInsertRowid),"payment_applied","receivables.engine",{payment_id:payment.id,invoice_id:invoice.id,amount_cents:input.amount_cents,journal_entry_id:journal.id});
    return invoiceRecord(invoice.id,date);
  }

  function createCreditMemo(input) {
    const invoice=invoiceRecord(input.invoice_id,input.credit_date); if(!invoice||invoice.status==="void")throw bad("Open invoice not found",404);
    requireAmount(input.amount_cents,"Credit amount"); if(input.amount_cents>invoice.balance_cents)throw bad("Credit exceeds invoice balance");
    if(!input.credit_date||!input.reason?.trim())throw bad("Credit date and reason are required");
    const contract=getContract(invoice.contract_id);
    const recognized=db.prepare(`SELECT COALESCE(SUM(rs.amount_cents),0) value FROM revenue_schedules rs JOIN performance_obligations po ON po.id=rs.obligation_id WHERE po.contract_id=? AND rs.status='recognized'`).get(invoice.contract_id).value;
    const netBilled=db.prepare(`SELECT COALESCE(SUM(i.amount_cents),0)-COALESCE((SELECT SUM(cm.amount_cents) FROM credit_memos cm JOIN invoices i2 ON i2.id=cm.invoice_id WHERE i2.contract_id=? AND cm.status='posted'),0) value FROM invoices i WHERE i.contract_id=? AND i.status<>'void'`).get(invoice.contract_id,invoice.contract_id).value;
    const deferredPart=Math.min(input.amount_cents,Math.max(0,netBilled-recognized)),revenuePart=input.amount_cents-deferredPart;
    const lines=[]; if(deferredPart)lines.push({account_id:accountId("2100"),debit_cents:deferredPart,credit_cents:0}); if(revenuePart)lines.push({account_id:accountId("4050"),debit_cents:revenuePart,credit_cents:0}); lines.push({account_id:accountId("1100"),debit_cents:0,credit_cents:input.amount_cents});
    const number=input.credit_number?.trim()||`CM-${Date.now()}`;
    if(db.prepare("SELECT id FROM credit_memos WHERE credit_number=?").get(number))throw bad("Credit memo number already exists",409);
    const draft=ledger.createDraft({date:input.credit_date,memo:`Credit ${number} — ${invoice.invoice_number}`,source:"credit_memo",entity_id:contract.entity_id,currency:contract.currency,lines},"receivables.engine");
    const journal=ledger.postJournal(draft.id,"receivables.engine");
    const result=db.prepare("INSERT INTO credit_memos(invoice_id,credit_number,credit_date,amount_cents,reason,journal_entry_id) VALUES(?,?,?,?,?,?)").run(invoice.id,number,input.credit_date,input.amount_cents,input.reason.trim(),journal.id);
    audit(db,"credit_memo",Number(result.lastInsertRowid),"credit_memo_posted","receivables.engine",{invoice_id:invoice.id,amount_cents:input.amount_cents,journal_entry_id:journal.id});
    return invoiceRecord(invoice.id,input.credit_date);
  }

  function voidInvoice(input) {
    const invoice=invoiceRecord(input.invoice_id,input.void_date);if(!invoice||invoice.status==="void")throw bad("Open invoice not found",404);
    if(invoice.applied_cents||invoice.refund_cents||invoice.write_off_cents)throw bad("Invoices with payments, refunds, or write-offs cannot be voided; issue a credit memo instead");
    if(invoice.balance_cents)createCreditMemo({invoice_id:invoice.id,credit_number:input.credit_number||`VOID-${invoice.invoice_number}`,credit_date:input.void_date,amount_cents:invoice.balance_cents,reason:input.reason||"Invoice voided"});
    db.prepare("UPDATE invoices SET status='void' WHERE id=?").run(invoice.id);audit(db,"invoice",invoice.id,"invoice_voided","receivables.engine",{void_date:input.void_date,reason:input.reason||"Invoice voided"});return invoiceRecord(invoice.id,input.void_date);
  }

  function writeOffInvoice(input) {
    const invoice=invoiceRecord(input.invoice_id,input.write_off_date); if(!invoice||invoice.status==="void")throw bad("Open invoice not found",404);
    requireAmount(input.amount_cents,"Write-off amount"); if(input.amount_cents>invoice.balance_cents)throw bad("Write-off exceeds invoice balance");
    if(!input.write_off_date||!input.reason?.trim())throw bad("Write-off date and reason are required");
    const draft=ledger.createDraft({date:input.write_off_date,memo:`AR write-off — ${invoice.invoice_number}`,source:"ar_write_off",entity_id:invoice.entity_id,lines:[
      {account_id:accountId("5350"),debit_cents:input.amount_cents,credit_cents:0},{account_id:accountId("1100"),debit_cents:0,credit_cents:input.amount_cents}
    ]},"receivables.engine");
    const journal=ledger.postJournal(draft.id,"receivables.engine");
    const result=db.prepare("INSERT INTO ar_write_offs(invoice_id,write_off_date,amount_cents,reason,journal_entry_id) VALUES(?,?,?,?,?)").run(invoice.id,input.write_off_date,input.amount_cents,input.reason.trim(),journal.id);
    audit(db,"write_off",Number(result.lastInsertRowid),"receivable_written_off","receivables.engine",{invoice_id:invoice.id,amount_cents:input.amount_cents,journal_entry_id:journal.id});
    return invoiceRecord(invoice.id,input.write_off_date);
  }

  function refundPayment(input) {
    const payment=paymentRecord(input.payment_id); if(!payment||payment.status!=="received")throw bad("Active payment not found",404);
    requireAmount(input.amount_cents,"Refund amount"); if(!input.refund_date||!input.reason?.trim())throw bad("Refund date and reason are required");
    const refundable=payment.amount_cents-payment.refunded_cents; if(input.amount_cents>refundable)throw bad("Refund exceeds payment amount");
    let invoice=null;
    if(input.invoice_id){invoice=invoiceRecord(input.invoice_id,input.refund_date);if(!invoice||invoice.customer_id!==payment.customer_id)throw bad("Refund invoice must belong to the payment customer");const paidOnInvoice=db.prepare("SELECT COALESCE(SUM(amount_cents),0) value FROM payment_applications WHERE payment_id=? AND invoice_id=? AND status='applied'").get(payment.id,invoice.id).value;const alreadyRefunded=db.prepare("SELECT COALESCE(SUM(amount_cents),0) value FROM customer_refunds WHERE payment_id=? AND invoice_id=? AND status='posted'").get(payment.id,invoice.id).value;if(input.amount_cents>paidOnInvoice-alreadyRefunded)throw bad("Refund exceeds the amount this payment applied to the invoice");}
    else {const unapplied=payment.amount_cents-payment.applied_cents-payment.refunded_cents;if(input.amount_cents>unapplied)throw bad("Select an invoice for a refund of applied cash");}
    const number=input.refund_number?.trim()||`RF-${Date.now()}`;
    if(db.prepare("SELECT id FROM customer_refunds WHERE refund_number=?").get(number))throw bad("Refund number already exists",409);
    const debitCode=invoice?"1100":"2150";
    const draft=ledger.createDraft({date:input.refund_date,memo:`Customer refund ${number}`,source:"customer_refund",entity_id:payment.entity_id,lines:[
      {account_id:accountId(debitCode),debit_cents:input.amount_cents,credit_cents:0},{account_id:accountId("1000"),debit_cents:0,credit_cents:input.amount_cents}
    ]},"receivables.engine");
    const journal=ledger.postJournal(draft.id,"receivables.engine");
    const result=db.prepare("INSERT INTO customer_refunds(payment_id,invoice_id,refund_number,refund_date,amount_cents,reason,journal_entry_id) VALUES(?,?,?,?,?,?,?)").run(payment.id,invoice?.id||null,number,input.refund_date,input.amount_cents,input.reason.trim(),journal.id);
    audit(db,"refund",Number(result.lastInsertRowid),"customer_refunded","receivables.engine",{payment_id:payment.id,invoice_id:invoice?.id||null,amount_cents:input.amount_cents,journal_entry_id:journal.id});
    return {refund_id:Number(result.lastInsertRowid),payment:paymentRecord(payment.id),invoice:invoice?invoiceRecord(invoice.id,input.refund_date):null};
  }

  function voidPayment(input) {
    const payment=paymentRecord(input.payment_id); if(!payment||payment.status!=="received")throw bad("Active payment not found",404);
    if(payment.refunded_cents)throw bad("A refunded payment cannot be voided");
    const date=input.void_date||new Date().toISOString().slice(0,10),unapplied=payment.amount_cents-payment.applied_cents;
    const lines=[];if(payment.applied_cents)lines.push({account_id:accountId("1100"),debit_cents:payment.applied_cents,credit_cents:0});if(unapplied)lines.push({account_id:accountId("2150"),debit_cents:unapplied,credit_cents:0});lines.push({account_id:accountId("1000"),debit_cents:0,credit_cents:payment.amount_cents});
    const draft=ledger.createDraft({date,memo:`Void customer payment ${payment.payment_number}`,source:"payment_void",entity_id:payment.entity_id,lines},"receivables.engine");const journal=ledger.postJournal(draft.id,"receivables.engine");
    db.prepare("UPDATE customer_payments SET status='void',void_journal_entry_id=? WHERE id=?").run(journal.id,payment.id);db.prepare("UPDATE payment_applications SET status='reversed',reversed_at=? WHERE payment_id=? AND status='applied'").run(date,payment.id);
    audit(db,"payment",payment.id,"payment_voided","receivables.engine",{journal_entry_id:journal.id});return paymentRecord(payment.id);
  }

  function openDispute(input) {
    const invoice=invoiceRecord(input.invoice_id,input.opened_date);if(!invoice||invoice.status==="void")throw bad("Open invoice not found",404);requireAmount(input.amount_cents,"Dispute amount");if(input.amount_cents>invoice.balance_cents-invoice.disputed_cents)throw bad("Dispute exceeds undisputed invoice balance");if(!input.opened_date||!input.reason?.trim())throw bad("Dispute date and reason are required");
    const result=db.prepare("INSERT INTO invoice_disputes(invoice_id,opened_date,amount_cents,reason) VALUES(?,?,?,?)").run(invoice.id,input.opened_date,input.amount_cents,input.reason.trim());audit(db,"dispute",Number(result.lastInsertRowid),"invoice_disputed","collections.user",{invoice_id:invoice.id,amount_cents:input.amount_cents});return invoiceRecord(invoice.id,input.opened_date);
  }
  function resolveDispute(input) {const dispute=db.prepare("SELECT * FROM invoice_disputes WHERE id=? AND status='open'").get(input.dispute_id);if(!dispute)throw bad("Open dispute not found",404);if(!input.resolved_date||!input.resolution?.trim())throw bad("Resolution date and notes are required");db.prepare("UPDATE invoice_disputes SET status=?,resolution=?,resolved_date=? WHERE id=?").run(input.status==="withdrawn"?"withdrawn":"resolved",input.resolution.trim(),input.resolved_date,dispute.id);audit(db,"dispute",dispute.id,"dispute_resolved","collections.user",{resolution:input.resolution});return invoiceRecord(dispute.invoice_id,input.resolved_date);}
  function addCollectionActivity(input) {if(!db.prepare("SELECT id FROM customers WHERE id=?").get(input.customer_id))throw bad("Customer not found",404);if(!input.activity_date||!input.notes?.trim())throw bad("Activity date and notes are required");if(input.invoice_id){const invoice=invoiceRecord(input.invoice_id,input.activity_date);if(!invoice||invoice.customer_id!==input.customer_id)throw bad("Invoice does not belong to customer");}const type=["email","call","promise_to_pay","dunning","note"].includes(input.activity_type)?input.activity_type:"note";const result=db.prepare("INSERT INTO collection_activities(customer_id,invoice_id,activity_date,activity_type,notes,next_action_date,status) VALUES(?,?,?,?,?,?,?)").run(input.customer_id,input.invoice_id||null,input.activity_date,type,input.notes.trim(),input.next_action_date||null,input.status==="completed"?"completed":"open");audit(db,"collection_activity",Number(result.lastInsertRowid),"collection_activity_logged","collections.user",{customer_id:input.customer_id,invoice_id:input.invoice_id||null,type});return db.prepare("SELECT * FROM collection_activities WHERE id=?").get(result.lastInsertRowid);}
  function completeCollectionActivity(input) {const activity=db.prepare("SELECT * FROM collection_activities WHERE id=?").get(input.activity_id);if(!activity)throw bad("Collection activity not found",404);db.prepare("UPDATE collection_activities SET status='completed' WHERE id=?").run(activity.id);audit(db,"collection_activity",activity.id,"collection_activity_completed","collections.user",{});return db.prepare("SELECT * FROM collection_activities WHERE id=?").get(activity.id);}

  function receivables(asOf=new Date().toISOString().slice(0,10)) {
    const invoices=listInvoices(asOf),payments=listPayments(asOf);
    const aging={current_cents:0,days_1_30_cents:0,days_31_60_cents:0,days_61_90_cents:0,days_90_plus_cents:0,total_cents:0,disputed_cents:0};
    for(const invoice of invoices.filter(i=>i.status!=="void"&&i.balance_cents>0)){aging.total_cents+=invoice.balance_cents;aging.disputed_cents+=invoice.disputed_cents;if(!invoice.days_past_due)aging.current_cents+=invoice.balance_cents;else if(invoice.days_past_due<=30)aging.days_1_30_cents+=invoice.balance_cents;else if(invoice.days_past_due<=60)aging.days_31_60_cents+=invoice.balance_cents;else if(invoice.days_past_due<=90)aging.days_61_90_cents+=invoice.balance_cents;else aging.days_90_plus_cents+=invoice.balance_cents;}
    const customers=db.prepare("SELECT id,name FROM customers ORDER BY name").all().map(customer=>{const rows=invoices.filter(i=>i.customer_id===customer.id&&i.status!=="void");return {...customer,billed_cents:rows.reduce((s,i)=>s+i.amount_cents,0),outstanding_cents:rows.reduce((s,i)=>s+i.balance_cents,0),overdue_cents:rows.filter(i=>i.days_past_due>0).reduce((s,i)=>s+i.balance_cents,0)};});
    const contracts=listContracts().map(contract=>{const rows=invoices.filter(i=>i.contract_id===contract.id&&i.status!=="void"),credits=rows.reduce((s,i)=>s+i.credit_cents,0),netBilled=rows.reduce((s,i)=>s+i.amount_cents-i.credit_cents,0);return {contract_id:contract.id,contract_number:contract.contract_number,customer_name:contract.customer_name,allocated_cents:contract.allocated_cents,recognized_cents:contract.recognized_cents,billed_cents:rows.reduce((s,i)=>s+i.amount_cents,0),credit_cents:credits,net_billed_cents:netBilled,unbilled_cents:Math.max(0,contract.recognized_cents-netBilled),deferred_cents:Math.max(0,netBilled-contract.recognized_cents),outstanding_cents:rows.reduce((s,i)=>s+i.balance_cents,0)};});
    const glAr=db.prepare(`SELECT COALESCE(SUM(l.debit_cents-l.credit_cents),0) value FROM journal_entries j JOIN journal_lines l ON l.entry_id=j.id JOIN accounts a ON a.id=l.account_id WHERE j.status='posted' AND a.code='1100' AND j.entry_date<=?`).get(asOf).value;
    const subledgerAr=invoices.filter(i=>i.invoice_date<=asOf&&i.status!=="void").reduce((s,i)=>s+i.balance_cents,0);
    const glUnapplied=db.prepare(`SELECT COALESCE(SUM(l.credit_cents-l.debit_cents),0) value FROM journal_entries j JOIN journal_lines l ON l.entry_id=j.id JOIN accounts a ON a.id=l.account_id WHERE j.status='posted' AND a.code='2150' AND j.entry_date<=?`).get(asOf).value;
    const unapplied=payments.filter(p=>p.status==='received').reduce((s,p)=>s+p.unapplied_cents,0);
    const collections=db.prepare(`SELECT ca.*,cu.name customer_name,i.invoice_number FROM collection_activities ca JOIN customers cu ON cu.id=ca.customer_id LEFT JOIN invoices i ON i.id=ca.invoice_id ORDER BY COALESCE(ca.next_action_date,ca.activity_date),ca.id DESC`).all();
    const disputes=db.prepare(`SELECT d.*,i.invoice_number,cu.name customer_name FROM invoice_disputes d JOIN invoices i ON i.id=d.invoice_id JOIN contracts c ON c.id=i.contract_id JOIN customers cu ON cu.id=c.customer_id ORDER BY d.id DESC`).all();
    return {as_of:asOf,invoices,payments,aging,customers,contracts,collections,disputes,reconciliation:{gl_ar_cents:glAr,subledger_ar_cents:subledgerAr,ar_difference_cents:glAr-subledgerAr,gl_unapplied_cents:glUnapplied,subledger_unapplied_cents:unapplied,unapplied_difference_cents:glUnapplied-unapplied,balanced:glAr===subledgerAr&&glUnapplied===unapplied}};
  }
  ...
  function deferredRollforward() { const billed=db.prepare("SELECT COALESCE(SUM(amount_cents),0) value FROM invoices WHERE status<>'void'").get().value-db.prepare("SELECT COALESCE(SUM(cm.amount_cents),0) value FROM credit_memos cm JOIN invoices i ON i.id=cm.invoice_id WHERE cm.status='posted' AND i.status<>'void'").get().value; const recognized=db.prepare("SELECT COALESCE(SUM(amount_cents),0) value FROM revenue_schedules WHERE status='recognized'").get().value; return {opening_cents:0,billings_cents:billed,revenue_cents:recognized,ending_cents:Math.max(0,billed-recognized),contract_asset_cents:Math.max(0,recognized-billed)}; }
  ...
  function metrics() {
    ...
    const billings=db.prepare("SELECT COALESCE(SUM(amount_cents),0) value FROM invoices WHERE status<>'void'").get().value-db.prepare("SELECT COALESCE(SUM(cm.amount_cents),0) value FROM credit_memos cm JOIN invoices i ON i.id=cm.invoice_id WHERE cm.status='posted' AND i.status<>'void'").get().value;
  ...
  return {listContracts,getContract,createContract,createInvoice,listInvoices,recordPayment,applyPayment,createCreditMemo,voidInvoice,writeOffInvoice,refundPayment,voidPayment,openDispute,resolveDispute,addCollectionActivity,completeCollectionActivity,receivables,recordUsage,updateMilestone,recognizeThrough,modifyContract,addSoftwareProject,revenueWaterfall,deferredRollforward,rpo,metrics,cashFlow,consolidation,revalueFx,postEliminations,
...
function requireAmount(value,label){if(!Number.isSafeInteger(value)||value<=0)throw bad(`${label} must be a positive whole number of cents`);}
