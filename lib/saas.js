...
export function createSaasRepository(db, ledger) {
  ...
  function createContract(input) {
    ...
    try {
      ...
      audit(db, "contract", contractId, "asc606_contract_created", "demo.user", {
        allocatable,
        totalSsp,
  ...
  function createInvoice(input) {
    ...
    const result = db
      .prepare(
        "INSERT INTO invoices(contract_id,invoice_number,invoice_date,due_date,amount_cents,journal_entry_id) VALUES(?,?,?,?,?,?)",
      )
      .run(
        input.contract_id,
    ...
    audit(db, "invoice", Number(result.lastInsertRowid), "invoice_posted", "billing.engine", {
  ...
  function recordPayment(input) {
    ...
    audit(db, "payment", paymentId, "payment_received", "receivables.engine", {
  ...
  function voidInvoice(input) {
    ...
    audit(db, "invoice", invoice.id, "invoice_voided", "receivables.engine", {
  ...
  function refundPayment(input) {
    ...
    audit(db, "refund", Number(result.lastInsertRowid), "customer_refunded", "receivables.engine", {
      payment_id: payment.id,
  ...
  function voidPayment(input) {
    ...
    audit(db, "payment", payment.id, "payment_voided", "receivables.engine", {
      journal_entry_id: journal.id,
    });
  ...
  function recognizeCommissionThrough(through) {
    ...
    for (const row of rows) {
      ...
      const j = ledger.postJournal(draft.id, "commission.engine");
      db.prepare(
  ...
  function recognizeSoftwareThrough(through) {
    ...
    for (const row of rows) {
      ...
      const j = ledger.postJournal(draft.id, "software.engine");
      db.prepare(
  ...
  function revalueFx(asOf) {
    ...
    for (const entity of entities) {
      ...
      posted.push(ledger.postJournal(d.id, "fx.engine").id);
    }
  ...
  function postEliminations(asOf) {
    ...
    const j = ledger.postJournal(d.id, "consolidation.engine");
...
function audit(db, type, id, action, actor, payload) {
  ...
  ).run(type, id, action, actor, JSON.stringify(payload));
}
