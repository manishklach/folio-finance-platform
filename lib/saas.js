...
export function createSaasRepository(db, ledger) {
  ...
  function createContract(input) {
    ...
    const transactionPrice = Number(input.transaction_price_cents);
    const variableConsideration = Number(input.variable_consideration_cents || 0);
    const constraintPercent = Number(input.constraint_percent ?? 100);
    if (!Number.isSafeInteger(transactionPrice) || transactionPrice <= 0)
      throw bad("Transaction price must be a positive whole number of cents");
    if (!Number.isSafeInteger(variableConsideration) || variableConsideration < 0)
      throw bad("Variable consideration must be a non-negative whole number of cents");
    if (!Number.isFinite(constraintPercent) || constraintPercent < 0 || constraintPercent > 100)
      throw bad("Constraint percent must be between 0 and 100");
    const constraintBasisPoints = Math.round(constraintPercent * 100);
    if (Math.abs(constraintPercent * 100 - constraintBasisPoints) > 1e-8)
      throw bad("Constraint percent supports at most two decimal places");
    const weights = input.obligations.map((obligation) => Number(obligation.ssp_cents));
    if (weights.some((weight) => !Number.isSafeInteger(weight) || weight <= 0))
      throw bad("Every standalone selling price must be a positive whole number of cents");
    const constrainedVariable = Number(
      (BigInt(variableConsideration) * BigInt(constraintBasisPoints) + 5000n) / 10000n,
    );
    const allocatable = transactionPrice + constrainedVariable;
    if (!Number.isSafeInteger(allocatable)) throw bad("Allocatable consideration is too large");
    const allocations = allocateCentsByWeight(allocatable, weights);
    const totalSsp = weights.reduce((sum, weight) => sum + weight, 0);
    if (!Number.isSafeInteger(totalSsp)) throw bad("Total standalone selling price is too large");
    let contractId;
    const ownsTransaction = !db.isTransaction;
    if (ownsTransaction) db.exec("BEGIN IMMEDIATE");
    try {
  ...
  function modifyContract(input) {
    return atomic(() => {
      const contract = getContract(input.contract_id);
      if (!contract) throw bad("Contract not found", 404);
      if (!["cancellation", "extension", "price_change"].includes(input.kind))
        throw bad("Modification kind must be cancellation, extension, or price_change");
      const priceChange = Number(input.price_change_cents || 0);
      if (!Number.isSafeInteger(priceChange))
        throw bad("Price change must be a whole number of cents");
      if (input.kind === "extension" && !input.new_end_date)
        throw bad("An extension requires a new end date");
      if (input.kind === "price_change" && !priceChange)
        throw bad("A price change modification requires a non-zero amount");
      const priceDeltas = priceChange
        ? allocateSignedCentsByWeight(
            priceChange,
            contract.obligations.map(({ ssp_cents }) => ssp_cents),
          )
        : [];
      if (priceChange && contract.transaction_price_cents + priceChange <= 0)
        throw bad("A price change must leave a positive transaction price");
      if (
        priceDeltas.some(
          (delta, index) => contract.obligations[index].allocated_price_cents + delta < 0,
        )
      )
        throw bad("A price change cannot create a negative obligation allocation");
      db.prepare(
        "INSERT INTO contract_modifications(contract_id,effective_date,kind,description,price_change_cents) VALUES(?,?,?,?,?)",
      ).run(input.contract_id, input.effective_date, input.kind, input.description, priceChange);
      if (input.kind === "cancellation") {
        ...
        for (const po of contract.obligations.filter(
          ...
          db.prepare(
            "DELETE FROM revenue_schedules WHERE obligation_id=? AND status='pending'",
          ).run(po.id);
        ...
        ).run(priceChange, input.contract_id);
        contract.obligations.forEach((po, index) => {
          const delta = priceDeltas[index];
          db.prepare(
  ...
  function deferredRollforward(asOf = "9999-12-31") {
    const billed =
      db
        .prepare(
          "SELECT COALESCE(SUM(amount_cents),0) value FROM invoices WHERE status<>'void' AND invoice_date<=?",
        )
        .get(asOf).value -
      db
        .prepare(
          "SELECT COALESCE(SUM(cm.amount_cents),0) value FROM credit_memos cm JOIN invoices i ON i.id=cm.invoice_id WHERE cm.status='posted' AND i.status<>'void' AND cm.credit_date<=?",
        )
        .get(asOf).value;
    const recognized = db
      .prepare(
        `SELECT COALESCE(SUM(rs.amount_cents),0) value FROM revenue_schedules rs
         JOIN journal_entries j ON j.id=rs.journal_entry_id WHERE rs.status='recognized' AND j.entry_date<=?`,
      )
      .get(asOf).value;
    return {
      opening_cents: 0,
  ...
  function rpo(asOf = new Date().toISOString().slice(0, 10)) {
    const start = asOf.slice(0, 7);
    const end = addMonths(asOf, 12).slice(0, 7);
    return db
      .prepare(
        `SELECT COALESCE(SUM(rs.amount_cents),0) total_cents,COALESCE(SUM(CASE WHEN rs.period<=? THEN rs.amount_cents ELSE 0 END),0) next_12_months_cents FROM revenue_schedules rs JOIN performance_obligations po ON po.id=rs.obligation_id JOIN contracts c ON c.id=po.contract_id WHERE rs.status='pending' AND rs.period>=? AND c.status='active'`,
      )
      .get(end, start);
  }
  ...
  function cashFlow(asOf = "9999-12-31", from = "0000-01-01") {
    const rows = db
      .prepare(
        `SELECT j.id,j.memo,l.debit_cents,l.credit_cents,a.code,a.type FROM journal_entries j JOIN journal_lines l ON l.entry_id=j.id JOIN accounts a ON a.id=l.account_id WHERE j.status='posted' AND j.entry_date BETWEEN ? AND ?`,
      )
      .all(from, asOf);
    ...
    for (const lines of byEntry.values()) {
      ...
      if (codes.has("1050")) continue;
      const equityDistribution = db
        .prepare(
          "SELECT dividends_cents,dividend_return_of_capital_cents FROM equity_method_periods WHERE journal_entry_id=?",
        )
        .get(lines[0].id);
      if (equityDistribution && change > 0) {
        const returnOfCapital = equityDistribution.dividend_return_of_capital_cents || 0;
        operating += change - returnOfCapital;
        investing += returnOfCapital;
        continue;
      }
      const financingPrincipal = lines
        .filter((line) => ["2400", "2260"].includes(line.code))
        .reduce((sum, line) => sum + line.debit_cents - line.credit_cents, 0);
      if (change < 0 && financingPrincipal > 0) {
        financing -= financingPrincipal;
        operating += change + financingPrincipal;
      } else if (["3000", "3100", "2400", "2260"].some((code) => codes.has(code)))
        financing += change;
      else if (
        [
          "1250",
...
export function allocateCentsByWeight(total, weights) {
  if (!Number.isSafeInteger(total) || total < 0)
    throw bad("Allocation total must be a non-negative whole number of cents");
  if (
    !Array.isArray(weights) ||
    !weights.length ||
    weights.some((weight) => !Number.isSafeInteger(weight) || weight <= 0)
  )
    throw bad("Allocation weights must be positive whole numbers");
  const totalWeight = weights.reduce((sum, weight) => sum + BigInt(weight), 0n);
  const shares = weights.map((weight, index) => {
    const numerator = BigInt(total) * BigInt(weight);
    return {
      index,
      cents: Number(numerator / totalWeight),
      remainder: numerator % totalWeight,
    };
  });
  let residual = total - shares.reduce((sum, share) => sum + share.cents, 0);
  ...
  return shares.sort((left, right) => left.index - right.index).map(({ cents }) => cents);
}

export function allocateSignedCentsByWeight(total, weights) {
  if (!Number.isSafeInteger(total))
    throw bad("Signed allocation total must be a whole number of cents");
  const sign = total < 0 ? -1 : 1;
  return allocateCentsByWeight(Math.abs(total), weights).map((amount) => amount * sign);
}
