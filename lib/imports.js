...
export function createImportsRepository(db, ledger) {
  ...
  function stageImport(input) {
    ...
    const mapping = validateMapping(definition, parsed.headers, input.mapping || {});
    ...
    try {
      db.prepare(
        `INSERT INTO import_batches(id,template_key,template_version,filename,file_sha256,mapping_json,options_json,row_count,valid_count,error_count,duplicate_count,created_by)
         VALUES(?,?,?,?,?,?,?,?,?,?,?,?)`,
      ).run(
        id,
        input.template_key,
  ...
  function importBatch(id) {
    ...
    return {
      ...
      rows: db
        .prepare("SELECT * FROM import_rows WHERE batch_id=? ORDER BY row_number LIMIT 250")
        .all(id)
        .map(hydrateRow),
      exceptions: db
  ...
  function importExceptions() {
    return db
      .prepare(
        "SELECT * FROM import_exceptions ORDER BY CASE status WHEN 'open' THEN 0 WHEN 'acknowledged' THEN 1 ELSE 2 END,created_at DESC",
      )
      .all();
  }
