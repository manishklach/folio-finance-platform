...
export function createImportsRepository(db, ledger) {
  ...
  function stageImport(input) {
    ...
    try {
      db.prepare(
        `INSERT INTO import_batches(id,template_key,template_version,filename,file_sha256,mapping_json,mapping_profile_id,mapping_profile_version,options_json,row_count,valid_count,error_count,duplicate_count,created_by)
         VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
      ).run(
        id,
        input.template_key,
