...
export async function proposeJournal(description, accounts) {
  if (!description?.trim())
    throw Object.assign(new Error("Describe the transaction first"), { statusCode: 400 });
...
async function proposeWithOpenAI(description, accounts) {
  ...
  if (proposal.lines.some((line) => !allowed.has(line.account_id)))
    throw new Error("AI selected an unknown account");
...
function fallbackProposal(description, accounts) {
  ...
  if (!amount)
    throw Object.assign(new Error("Include an amount, for example: Paid $1,200 for AWS hosting"), {
      statusCode: 400,
    });
  ...
  if (/aws|azure|gcp|cloud|hosting|compute/.test(text)) {
    debit = account("5000");
    rationale = "Classified as cloud infrastructure expense.";
  } else if (/payroll|salary|wage/.test(text)) {
    debit = account("5200");
    rationale = "Classified as payroll expense.";
  } else if (/marketing|advert|campaign/.test(text)) {
    debit = account("5300");
    rationale = "Classified as marketing expense.";
  } else if (/subscription|customer|revenue|sale|invoice/.test(text)) {
    debit = /invoice|receivable/.test(text) ? account("1100") : account("1000");
    credit = account("4000");
    rationale = "Classified as subscription revenue; confirm revenue timing before posting.";
  } else if (/prepaid|annual/.test(text)) {
    debit = account("1200");
    rationale = "Classified as a prepaid asset; establish an amortization schedule after review.";
  }
  if (/on account|unpaid|bill|payable/.test(text) && credit?.type !== "revenue")
    credit = account("2000");
