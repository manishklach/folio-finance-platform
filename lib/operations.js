...
export function createOperationsRepository(db, ledger, options = {}) {
  ...
  function completeCloseItem(input) {
    ...
    if (input.item_key === "bank_reconciled") {
      ...
      const unresolvedPayrollSettlement = db
        .prepare(
          `SELECT r.check_date FROM payroll_settlements s JOIN payroll_runs r ON r.id=s.payroll_run_id
           WHERE r.status NOT IN ('superseded','removed') AND s.status<>'reconciled'`,
        )
        .all()
        .some((item) => fiscalPeriod(item.check_date) === input.period);
      if (
        ...
        unresolvedPayrollSettlement
      )
        throw bad(
          "Bank or payment-settlement reconciliation exceptions, including payroll-settlement exceptions, for this period must be resolved before sign-off",
          409,
        );
    }
    if (input.item_key === "accruals_posted") {
      const unresolvedPayrollSource = db
        .prepare(
          `SELECT effective_at FROM integration_records
           WHERE object_type='payroll_run' AND status IN ('staged','error') AND effective_at IS NOT NULL`,
        )
        .all()
        .some((item) => fiscalPeriod(item.effective_at.slice(0, 10)) === input.period);
      const unpostedPayroll = db
        .prepare(
          `SELECT r.check_date FROM payroll_runs r LEFT JOIN journal_entries j ON j.id=r.accounting_journal_id
           WHERE r.status IN ('draft','reversal_draft','exception') AND COALESCE(j.status,'missing')<>'posted'`,
        )
        .all()
        .some((item) => fiscalPeriod(item.check_date) === input.period);
      if (unresolvedPayrollSource || unpostedPayroll)
        throw bad(
          "Payroll source exceptions and accrual or reversal drafts for this period must be posted before sign-off",
          409,
        );
    }
    if (input.item_key === "revenue_reviewed") {
      const unresolvedClosedWonDeal = db
        .prepare(
          `SELECT json_extract(normalized_json,'$.close_date') close_date
           FROM integration_records
           WHERE object_type='hubspot_deal' AND status IN ('staged','error')
           AND json_extract(normalized_json,'$.is_closed_won')=1`,
        )
        .all()
        .some(
          (item) =>
            item.close_date && fiscalPeriod(String(item.close_date).slice(0, 10)) === input.period,
        );
      const unresolvedProposal = db
        .prepare(
          "SELECT signed_date FROM crm_contract_proposals WHERE status IN ('prepared','approved','exception')",
        )
        .all()
        .some((item) => fiscalPeriod(item.signed_date) === input.period);
      if (unresolvedClosedWonDeal || unresolvedProposal)
        throw bad(
          "Closed-won CRM source versions and contract proposals for this period must be resolved or applied before revenue sign-off",
          409,
        );
    }
    db.prepare(
