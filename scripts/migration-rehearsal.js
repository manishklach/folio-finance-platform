...
try {
  ...
  assert.deepEqual(migratePlatform(db), [7, 8]);
  const status = platformMigrationStatus(db);
  assert.equal(status.valid, true);
  assert.deepEqual(status.pending, []);
  process.stdout.write(
    `${JSON.stringify({ database: "temporary", oldest_supported: 1, target: status.latest, rows: volume, rollback_reapplied: 8, integrity: "ok" })}\n`,
  );
} finally {
  db.close();
